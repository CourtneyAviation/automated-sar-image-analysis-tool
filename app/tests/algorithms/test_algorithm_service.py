...
@patch('app.helpers.MetaDataHelper.MetaDataHelper.transferExifPiexif')
@patch('pathlib.Path.parents', new_callable=MagicMock)
@patch('builtins.open', new_callable=mock_open)
@patch('PIL.Image.open', return_value=MagicMock())
def test_store_image(mock_image_open, mock_open, mock_path_parents, mock_transferExifPiexif, mock_platform, mock_imencode, mock_exists, mock_makedirs, algorithm_service):
